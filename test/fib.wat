(module
  ...
  (data $d496 (i32.const 496) "finally")
(data $d468 (i32.const 468) "null")
(data $d300 (i32.const 300) "n")
(data $d372 (i32.const 372) "Promise resolver is not a function")
(data $d364 (i32.const 364) "Promise")
(data $d452 (i32.const 452) "undefined")
(data $d504 (i32.const 504) "false")
(data $d308 (i32.const 308) " ")
(data $d312 (i32.const 312) "catch")
(data $d444 (i32.const 444) "true")
(data $d448 (i32.const 448) "then")
(data $d472 (i32.const 472) "number")
(data $d464 (i32.const 464) "\n")
(data $d488 (i32.const 488) "object")
(data $d428 (i32.const 428) "function")
(data $d320 (i32.const 320) "Promise resolver undefined is not a function")
(data $d480 (i32.const 480) "boolean")
(data $d436 (i32.const 436) "string")
(data $d304 (i32.const 304) "fib")
(data $d408 (i32.const 408) "error encountered")
  ...
  (func $Promise-constructor
    ...
    (i32.eqz)
    (if (then
      (call $new_static_string (i32.const 320) (i32.const 44))
      (throw $JSException)
    ))
    ...
    (if (ref.test (ref $Function))
      ...
      (else
        ...
        (call $new_static_string (i32.const 372) (i32.const 34))
        (throw $JSException)
      )
    )
...
;;    )
  ...
  (func $type_of (param $arg anyref) (result (ref $StaticString))
    (if (ref.test nullref (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 452) (i32.const 9)))
      )
    )
    ...
    (if (ref.test i31ref (local.get $arg))
      (then
        (if (i32.or
          ...
          (then
            (return (call $new_static_string (i32.const 480) (i32.const 7)))
          )
          (else
            (return (call $new_static_string (i32.const 452) (i32.const 9)))
          )
    ...
    (if (ref.test (ref $Number) (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 472) (i32.const 6))))
    )
    ...
    (if (i32.or 
      ...
      (then
        (return (call $new_static_string (i32.const 488) (i32.const 6))))
    )
    ...
    (if (ref.test (ref $Function) (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 428) (i32.const 8))))
    )
...
(func $reverseBytes (param $start i32) (param $length i32) (result i32)
  ...
  (func $log (param $arguments (ref $JSArgs))
    ...
    (loop $process_args
      (block $process_arg
        ...
        (i32.store (local.get $iovectors_offset) (i32.const 308))
        (i32.store 
          (i32.add (local.get $iovectors_offset) (i32.const 4))
          (i32.const 1)
        )
    ...
    (i32.store (local.get $iovectors_offset) (i32.const 464))
    (i32.store 
      (i32.add (local.get $iovectors_offset) (i32.const 4))
      (i32.const 2)
    )
  ...
  ;; Bookmark
  (elem declare func $fib-Ze8RK5H)
(elem declare func $init)
  (func $fib-Ze8RK5H (param $parentScope (ref $Scope)) (param $this anyref) (param $arguments (ref $JSArgs)) (result anyref)
  (local $lhs-1 anyref)
  (local $rhs-1 anyref)
  (local $temp_arg-2 anyref)
  (local $scope (ref $Scope))
  (local $function-1 anyref)
  (local $function-2 anyref)
  (local $call_arguments-1 (ref $JSArgs))
  (local $call_arguments-2 (ref $JSArgs))
  (local $temp_arg-1 anyref)
  (call $new_scope (local.get $parentScope))
  (local.set $scope)
  (call $declare_variable (local.get $scope) (i32.const 300) (array.get $JSArgs (local.get $arguments) (i32.const 0)) (i32.const 3))
  (call $get_variable (local.get $scope) (i32.const 300))
(local.set $lhs-1)
(call $new_number (f64.const 2))
(local.set $rhs-1)
(local.get $lhs-1)
(local.get $rhs-1)
(call $less_than)

(call $cast_ref_to_i32_bool)
(if  (then (call $get_variable (local.get $scope) (i32.const 300))
return
))

  (call $add (array.new $JSArgs (ref.null any) (i32.const 1))
(local.set $call_arguments-1)
(call $sub (call $get_variable (local.get $scope) (i32.const 300)) (call $new_number (f64.const 1)))
(local.set $temp_arg-1)
(array.set $JSArgs (local.get $call_arguments-1) (i32.const 0) (local.get $temp_arg-1))

(call $get_variable (local.get $scope) (i32.const 304))
(local.set $function-1)
(call $call_function (local.get $function-1) (ref.null any) (local.get $call_arguments-1))
 (array.new $JSArgs (ref.null any) (i32.const 1))
(local.set $call_arguments-2)
(call $sub (call $get_variable (local.get $scope) (i32.const 300)) (call $new_number (f64.const 2)))
(local.set $temp_arg-2)
(array.set $JSArgs (local.get $call_arguments-2) (i32.const 0) (local.get $temp_arg-2))

(call $get_variable (local.get $scope) (i32.const 304))
(local.set $function-2)
(call $call_function (local.get $function-2) (ref.null any) (local.get $call_arguments-2))
)
return
...
(local.set $scope)
  ...
  (call $declare_variable (local.get $scope) (i32.const 304) (call $new_function (local.get $scope) (ref.func $fib-Ze8RK5H) (ref.null any)) (i32.const 2))
  (array.new $JSArgs (ref.null any) (i32.const 1))
(local.set $call_arguments-1)
(array.new $JSArgs (ref.null any) (i32.const 1))
(local.set $call_arguments-2)
(call $new_number (f64.const 40))
(local.set $temp_arg-2)
(array.set $JSArgs (local.get $call_arguments-2) (i32.const 0) (local.get $temp_arg-2))

(call $get_variable (local.get $scope) (i32.const 304))
(local.set $function-1)
(call $call_function (local.get $function-1) (ref.null any) (local.get $call_arguments-2))
...
(drop)
  ...
  (func $install-globals
    ...
    (call $set_variable (local.get $scope) (i32.const 364) (local.get $promise-constructor))
  )
  ...
  (func $outer_init (result i32)
    ...
    try
      ...
      (call $new_static_string (i32.const 408) (i32.const 17))
      (local.get $error)
      (call $create-arguments-2)
      (call $log)
      (return (i32.const 1))
