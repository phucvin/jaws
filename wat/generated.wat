(module
  ...
  (data $d444 (i32.const 444) "Promise resolver is not a function")
(data $d484 (i32.const 484) "Promise")
(data $d300 (i32.const 300) "a")
(data $d372 (i32.const 372) "null")
(data $d420 (i32.const 420) "undefined")
(data $d316 (i32.const 316) "number")
(data $d492 (i32.const 492) "true")
(data $d376 (i32.const 376) "object")
(data $d304 (i32.const 304) "b")
(data $d308 (i32.const 308) "string")
(data $d496 (i32.const 496) "false")
(data $d392 (i32.const 392) "function")
(data $d324 (i32.const 324) "Promise resolver undefined is not a function")
(data $d504 (i32.const 504) "boolean")
(data $d440 (i32.const 440) "then")
(data $d368 (i32.const 368) " ")
(data $d384 (i32.const 384) "finally")
(data $d432 (i32.const 432) "catch")
(data $d480 (i32.const 480) "\n")
(data $d400 (i32.const 400) "error encountered")
  ...
  (func $Promise-constructor
    ...
    (i32.eqz)
    (if (then
      (call $new_static_string (i32.const 324) (i32.const 44))
      (throw $JSException)
    ))
    ...
    (if (ref.test (ref $Function))
      ...
      (else
        ...
        (call $new_static_string (i32.const 444) (i32.const 34))
        (throw $JSException)
      )
    )
...
;;    )
  ...
  (func $type_of (param $arg anyref) (result (ref $StaticString))
    (if (ref.test nullref (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 420) (i32.const 9)))
      )
    )
    ...
    (if (ref.test (ref $Number) (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 316) (i32.const 6))))
    )
    ...
    (if (i32.or 
      ...
      (then
        (return (call $new_static_string (i32.const 376) (i32.const 6))))
    )

    (if (ref.test (ref $StaticString) (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 308) (i32.const 6))))
    )

    (if (ref.test (ref $Function) (local.get $arg))
      (then
        (return (call $new_static_string (i32.const 392) (i32.const 8))))
    )
...
(func $reverseBytes (param $start i32) (param $length i32) (result i32)
  ...
  (func $log (param $arguments (ref $JSArgs))
    ...
    (i32.store (local.get $iovectors_offset) (i32.const 480))
    (i32.store 
      (i32.add (local.get $iovectors_offset) (i32.const 4))
      (i32.const 2)
    )
...
(drop)
  ...
  (func $install-globals
    ...
    (call $set_variable (local.get $scope) (i32.const 484) (local.get $promise-constructor))
  )
  ...
  (func $outer_init (result i32)
    ...
    try
      ...
      (call $new_static_string (i32.const 400) (i32.const 17))
      (local.get $error)
      (call $create-arguments-2)
      (call $log)
      (return (i32.const 1))
